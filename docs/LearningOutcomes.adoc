...
* Assume you are planing to expand the functionality of the TodoList (but keep it as a CLI application).
What other user stories do you think TodoList should support? Add those user stories to the `DeveloperGuide.adoc`.
...
Assume that TodoList confirms the change with the user before carrying out the operation.
...
* Currently, TodoList data are saved to the file after every command.
...
* Do some enhancements to the TodoList GUI. e.g. add an application icon, change font size/style
...
=== Exercise: Enhance TodoList

* Enhance TodoList in some way. e.g. add a new command
